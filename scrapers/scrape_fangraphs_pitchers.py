...
def scrape_fangraphs_pitchers():
    ...
    for tr in table.find("tbody").find_all("tr"):
        ...
        for i, cell in enumerate(cells):
            ...
            # Special handling for player name column (usually second column)
            if i == 1 or 'player' in headers[i]:
    ...
    numeric_cols = ['overall_rank', 'WAR', 'ERA', 'WHIP', 'K_per_9', 'dynasty_value']
    ...
    df.to_csv("fangraphs_pitchers_rankings.csv", index=False)
    print("Saved FanGraphs pitchers rankings to fangraphs_pitchers_rankings.csv")

if __name__ == "__main__":
    scrape_fangraphs_pitchers()
